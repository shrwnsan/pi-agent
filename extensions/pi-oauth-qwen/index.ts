 ...
 *
 * Based on the upstream custom-provider-qwen-cli example with security
 * hardening applied (domain allowlisting, HTTPS enforcement, error
 * sanitization, expiry floor).
 *
 * Usage:
...
const QWEN_ALLOWED_DOMAINS = [/\.aliyuncs\.com$/, /\.qwen\.ai$/];
...
function truncate(text: string, max = MAX_ERROR_RESPONSE_LENGTH): string {
...
async function loginQwen(callbacks: OAuthLoginCallbacks): Promise<OAuthCredentials> {
	...
	// Calculate expiry with 5-minute buffer, but ensure a minimum lifetime
	const expiresAt = Math.max(
		Date.now() + MIN_TOKEN_LIFETIME_MS,
...
function getQwenBaseUrl(resourceUrl?: string): string {
	...
	try {
		...
		if (!QWEN_ALLOWED_DOMAINS.some((d) => d.test(parsed.hostname))) {
			return QWEN_DEFAULT_BASE_URL;
		}
