...
| `status-align` | Right-aligns pi's embedded `Working` spinner/status on the editor's top border (stock is left-aligned over a busy thinking area). Version-locked to pi 0.85.x, self-disables on seam changes. Toggle with `/status-align` |
