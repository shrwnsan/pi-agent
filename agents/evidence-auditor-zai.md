...
- Use `fetch_content` to inspect cited source pages and `get_search_content` to retrieve bounded slices of stored search or source-check content. Use `zai_web_search` only for targeted follow-up searches needed to verify or challenge a material claim.
